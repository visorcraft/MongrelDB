...
| Clojure `mongreldb_clojure` ✅ | Added `history-retention`, `history-retention-epochs`, `earliest-retained-epoch`, `set-history-retention-epochs`, and `last-epoch` to `src/visorcraft/mongreldb/core.clj`; `last-epoch` is captured from `/kit/txn` responses via a client atom. Column maps already pass typed scalars unchanged. | Extended `tests/visorcraft/mongreldb/live_test.clj` with the full static-default matrix, retention payload shape, live retention round-trip, and `AS OF EPOCH` time-travel coverage; updated `README.md` and `docs/quickstart.md`. |
...
| PowerShell `mongreldb_powershell` ✅ | Added `Set-MongrelDBHistoryRetention`, `Get-MongrelDBHistoryRetention`, and `Get-MongrelDBEarliestRetainedEpoch` to `src/MongrelDB.psm1` and exported them from `src/MongrelDB.psd1`. Keep `ConvertTo-MongrelDBCreateTableBody`. | Extended `tests/wire_shape_test.ps1` with the full static-default matrix and retention wire contract, added history-retention coverage to `tests/live_test.ps1`; updated `README.md` and `docs/quickstart.md`. CI build & test passed. |
| R `mongreldb_r` ✅ | Added `mongreldb_set_history_retention_epochs`, `mongreldb_history_retention_epochs`, and `mongreldb_earliest_retained_epoch` to `R/api.R`, plus `mongreldb_set_history_retention`/`mongreldb_history_retention` aliases, and exported them from `NAMESPACE`. Existing `encode_payload` preserves scalars; getter/setter values are validated against the exact-u64 representable range. | Extended `tests/testthat/test-create-table-wire-shape.R` with the full static-default matrix; added `tests/testthat/test-retention-wire.R`; added live retention + `AS OF EPOCH` coverage to `tests/testthat/test-live.R`; updated `README.md` and `docs/quickstart.md`. `R CMD check --no-manual` and the full `testthat` suite pass. |
| Ruby `mongreldb_ruby` ✅ | Added `set_history_retention_epochs`, `history_retention_epochs`, and `earliest_retained_epoch` to `lib/mongreldb.rb`; added `last_epoch` capture from `/kit/txn` responses and a `validate_u64!` guard. Hash/JSON bodies already preserve scalars. | Extended `spec/create_table_wire_shape_spec.rb` with the full static-default matrix; added `spec/history_retention_wire_spec.rb`; added live retention + `AS OF EPOCH` coverage to `test/live_test.rb`; updated `README.md` and `docs/quickstart.md`. |
| Scala `mongreldb_scala` ✅ | Added `setHistoryRetentionEpochs`, `historyRetentionEpochs`, and `earliestRetainedEpoch` to `src/main/scala/dev/visorcraft/mongreldb/MongrelDB.scala`; added `lastEpoch` capture from `/kit/txn` responses and a `u64ToLong` range guard. Existing `Json.scala` supports the scalar matrix. | Extended `src/test/scala/dev/visorcraft/mongreldb/CreateTableWireShapeTest.scala` with the full static-default matrix; added `src/test/scala/dev/visorcraft/mongreldb/HistoryRetentionWireShapeTest.scala`; added live retention + `AS OF EPOCH` coverage to `src/test/scala/dev/visorcraft/mongreldb/MongrelDBLiveTest.scala`; updated `README.md` and `docs/quickstart.md`. `sbt test` passes. |
| Swift `mongreldb_swift` ✅ | `setHistoryRetentionEpochs`, `historyRetentionEpochs`, and `earliestRetainedEpoch` are in `Sources/MongrelDB/MongrelDBClient.swift`; added `lastEpoch` capture from `/kit/txn` responses and a `UInt64` parsing helper. Existing `[String: Any]` bodies support `NSNull`. | Extended `Tests/MongrelDBTests/CreateTableWireShapeTests.swift` with the full static-default matrix; added `Tests/MongrelDBTests/HistoryRetentionWireShapeTests.swift`; added live retention + `AS OF EPOCH` coverage to `Tests/MongrelDBTests/MongrelDBLiveTests.swift`; updated `README.md` and `docs/quickstart.md`. Swift toolchain is not available locally; CI will verify. |
| Tcl `mongreldb_tcl` ✅ | `setHistoryRetentionEpochs`, `historyRetentionEpochs`, and `earliestRetainedEpoch` are namespace-exported in `src/mongreldb.tcl`; added `lastEpoch` capture from `/kit/txn` responses and per-handle `clientEpoch` state. `default_value_json` is kept for non-string scalars. | Extended `tests/wire_shape_test.tcl` with the full static-default matrix plus retention and `lastEpoch` wire-shape coverage; added live retention + `AS OF EPOCH` coverage to `tests/live_test.tcl`; updated `README.md` and `docs/quickstart.md`. `tclsh tests/wire_shape_test.tcl` and `tclsh tests/live_test.tcl` against a local daemon pass. |
| V `mongreldb_v` ✅ | Added `set_history_retention_epochs`, `history_retention_epochs`, and `earliest_retained_epoch` to `mongreldb/mongreldb.v`; switched epoch fields to `u64`; capture `last_epoch` from `/kit/txn` responses. Kept `Column.default_value`, `default_scalar`, and `default_expr`. | Extended `tests/wire_shape_test.v` with the full static-default matrix and retention payload shape; added live retention + `AS OF EPOCH` coverage to `tests/mongreldb_live_test.v`; updated `README.md`, `docs/quickstart.md`, `CONTRIBUTING.md`, and examples. |
| Zig `mongreldb_zig` ✅ | Added `setHistoryRetentionEpochs`, `historyRetentionEpochs`, and `earliestRetainedEpoch` to `Client` in `src/mongreldb.zig`; added `lastEpoch` capture from `/kit/txn` responses. Kept `Column.default_value`, `default_scalar`, and `default_expr`. | Extended `tests/wire_shape.zig` with the full static-default matrix and retention payload shape; added live retention + `AS OF EPOCH` coverage to `tests/live_test.zig`; updated `README.md` and `docs/quickstart.md`. |
