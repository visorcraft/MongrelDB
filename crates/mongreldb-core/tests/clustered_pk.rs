...
fn clustered_put_is_idempotent() {
    ...
    let (row_id_1, _) = table
        .put_returning(vec![(1, Value::Int64(42))])
        .unwrap();
    let (row_id_2, _) = table
        .put_returning(vec![(1, Value::Int64(42))])
        .unwrap();
    assert_eq!(
        row_id_1, row_id_2,
    ...
    let (row_id_3, _) = table
        .put_returning(vec![(1, Value::Int64(99))])
        .unwrap();
    assert_ne!(row_id_1, row_id_3, "different PKs must produce different RowIds");
}
...
fn clustered_table_stores_and_retrieves_data() {
    ...
    let mut table = Table::create(dir.path(), clustered_schema(), 1).unwrap();
    let (row_id, _) = table
        .put_returning(vec![(1, Value::Int64(7))])
        .unwrap();
    table.flush().unwrap();
