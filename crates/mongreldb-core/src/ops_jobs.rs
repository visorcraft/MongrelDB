...
impl OpsJobStore {
    ...
    pub fn open(dir: impl AsRef<Path>) -> Result<Self, OpsJobError> {
        ...
        let mut file: OpsJobsFile =
            serde_json::from_slice(&bytes).map_err(|e| OpsJobError::Storage(e.to_string()))?;
    ...
    fn persist(&self) -> Result<(), OpsJobError> {
        ...
        let bytes =
            serde_json::to_vec_pretty(&file).map_err(|e| OpsJobError::Storage(e.to_string()))?;
    ...
    pub fn start(&mut self, job_id: &str) -> Result<&OpsJob, OpsJobError> {
        ...
        match job.state {
            OpsJobState::Pending | OpsJobState::Paused => {
                ...
                self.persist()?;
                Ok(self.jobs.get(job_id).expect("present"))
            }
    ...
    pub fn mark_publish_fence(&mut self, job_id: &str) -> Result<&OpsJob, OpsJobError> {
        ...
        if job.state != OpsJobState::Running {
            return Err(OpsJobError::InvalidTransition {
                from: format!("{:?}", job.state),
                to: "publish_fence".into(),
            });
        }
        job.publish_fenced = true;
        job.progress = "publish fence".into();
        self.persist()?;
        Ok(self.jobs.get(job_id).expect("present"))
    }
    ...
    pub fn fail(&mut self, job_id: &str, error: impl Into<String>) -> Result<&OpsJob, OpsJobError> {
        ...
        job.state = OpsJobState::Failed;
        job.error = Some(error.into());
        job.progress = "failed".into();
        self.persist()?;
        Ok(self.jobs.get(job_id).expect("present"))
    }
    ...
    pub fn cancel(&mut self, job_id: &str) -> Result<&OpsJob, OpsJobError> {
        ...
        if job.publish_fenced {
            return Err(OpsJobError::TooLate {
                job_id: job_id.to_string(),
            });
        }
        job.state = OpsJobState::Cancelled;
        job.progress = "cancelled before publish".into();
        self.persist()?;
        Ok(self.jobs.get(job_id).expect("present"))
    }
...
pub enum OpsJobError {
    ...
    #[error("ops job {job_id} already crossed publish fence; cancel too late")]
    TooLate { job_id: String },
    #[error("ops job storage: {0}")]
    Storage(String),
}
#[cfg(test)]
mod tests {
    ...
    fn backup_job_is_resumable_across_phases() {
        ...
        store.advance_phase(&id, 2, "snap").unwrap();
    ...
    #[test]
    fn product_ops_kinds_exist() {
        assert_eq!(OpsJobKind::PRODUCT.len(), 9);
    }
    #[test]
    fn restart_resumes_running_as_pending() {
        let dir = tempfile::tempdir().unwrap();
        let mut store = OpsJobStore::open(dir.path()).unwrap();
        let job = store
            .submit_with(
                OpsJobKind::SplitTablet,
                BTreeMap::new(),
                Some("k".into()),
                42,
            )
        ...
        let id = job.job_id.clone();
        store.start(&id).unwrap();
        store.advance_phase(&id, 3, "x").unwrap();
        drop(store);
        let reopened = OpsJobStore::open(dir.path()).unwrap();
        let job = reopened.get(&id).unwrap();
        assert_eq!(job.state, OpsJobState::Pending);
        assert_eq!(job.phase, 3);
        assert_eq!(job.metadata_version, 42);
    }
    #[test]
    fn idempotent_key_returns_same_job() {
        let mut store = OpsJobStore::new();
        let a = store
            .submit_with(
    ...
    #[test]
    fn cancel_before_publish_and_too_late_after() {
        let mut store = OpsJobStore::new();
        let job = store
            .submit(OpsJobKind::MergeTablets, BTreeMap::new())
            .unwrap();
        let id = job.job_id.clone();
        store.start(&id).unwrap();
        ...
        store.start(&id2).unwrap();
        store.mark_publish_fence(&id2).unwrap();
        assert!(matches!(
            store.cancel(&id2),
            Err(OpsJobError::TooLate { .. })
        ));
    }
    #[test]
    fn accepted_response_shape() {
        let mut store = OpsJobStore::new();
        let job = store
            .submit_with(OpsJobKind::Backup, BTreeMap::new(), None, 7)
            .unwrap();
        let body = OpsJobStore::accepted_response(&job);
        assert_eq!(body["status"], "accepted");
        assert_eq!(body["state"], "pending");
        assert_eq!(body["metadata_version"], 7);
    }
    ...
    fn transfer_and_split_accept_return_job_id_and_cancel_before_publish() {
        ...
        assert!(body["job_id"]
            .as_str()
            .unwrap()
            .starts_with("ops-transfer_leader-"));
        ...
        assert!(split_body["job_id"]
            .as_str()
            .unwrap()
            .starts_with("ops-split_tablet-"));

        let id = split.job_id.clone();
        store.start(&id).unwrap();
        store.cancel(&id).unwrap();
        assert_eq!(store.get(&id).unwrap().state, OpsJobState::Cancelled);
        assert_eq!(store.get(&id).unwrap().progress, "cancelled before publish");
    }
    ...
    fn p16_x4_cancel_after_publish_fence_is_too_late() {
        ...
        let job = store
            .submit(
                OpsJobKind::SplitTablet,
                BTreeMap::from([("tablet_id".into(), "t9".into())]),
            )
            .unwrap();
        let id = job.job_id.clone();
        store.start(&id).unwrap();
    ...
    fn p16_x5_progress_and_errors_visible() {
        ...
        assert!(listed
            .iter()
            .any(|j| j.job_id == id && j.progress == "snapshot 40%"));
